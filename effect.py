...
class RandomSequenceIterator(BaseEffectIterator[RandomSequenceConfig]):
    def __init__(self, effect: "RandomSequence") -> None:
        ...
        self.characters_per_tick = max(
            int(self.config.speed * len(self.terminal._input_characters)), 1
        )
        self.build()
    ...
    def build(self) -> None:
        ...
        for character in self.terminal.get_characters():
            ...
            gradient_scn.apply_gradient_to_symbols(
                gradient, character.input_symbol, self.config.final_gradient_frames
            )
